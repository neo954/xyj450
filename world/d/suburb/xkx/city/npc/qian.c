...
int do_convert(string arg)
{
	...
	if (!to_ob && file_size("/d/obj/clone/money/" + to + ".c") < 0)
	{
	...
	bv2 = to_ob ? to_ob->query("base_value") : call_other("/d/obj/clone/money/" + to, "query", "base_value" );
	...
	if (!to_ob) {
		to_ob = new("/d/obj/clone/money/" + to);
...
int do_withdraw(string arg)
{
	...
	if (file_size("/d/obj/clone/money/" + what + ".c") < 0)
	{
		return notify_fail("你想取出什么钱？\n");
	}
	what = "/d/obj/clone/money/" + what;
